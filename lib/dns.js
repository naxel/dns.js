...
Client.prototype.lookup = function(host, callback) {
  return this.resolve4(host, function (err, addresses) {
    if (err) {
      callback(err);
    } else {
      callback(null, addresses.pop());
    }
  });
}
...
[ 'resolve', 'resolve4', 'resolve6', 'lookup' ].forEach(function(method) {
